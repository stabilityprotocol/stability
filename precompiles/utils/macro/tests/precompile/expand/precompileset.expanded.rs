	...
	Runtime: Get<u32>,
{
	...
	fn eip2612_domain_separator(
		...
		::core::panicking::panic_fmt(::core::fmt::Arguments::new_v1(
			&["not yet implemented: "],
			&[::core::fmt::ArgumentV1::new_display(
				&::core::fmt::Arguments::new_v1(&["eip2612_domain_separator"], &[]),
			)],
		))
	}
}
	...
	Runtime: Get<u32>,
{
	...
	pub fn execute(
		...
		let output = match self {
			...
			Self::__phantom(_, _) => ::core::panicking::panic_fmt(::core::fmt::Arguments::new_v1(
	...
	pub fn encode(self) -> ::sp_std::vec::Vec<u8> {
		use ::precompile_utils::EvmDataWriter;
		match self {
			Self::allowance { owner, spender } => EvmDataWriter::new_with_selector(3714247998u32)
				.write(owner)
				.write(spender)
				.build(),
			Self::approve { spender, value } => EvmDataWriter::new_with_selector(157198259u32)
				.write(spender)
				.write(value)
				.build(),
			Self::balance_of { who } => EvmDataWriter::new_with_selector(1889567281u32)
				.write(who)
				.build(),
			Self::burn { from, value } => EvmDataWriter::new_with_selector(2646777772u32)
				.write(from)
				.write(value)
				.build(),
			Self::clear_metadata {} => EvmDataWriter::new_with_selector(4021736498u32).build(),
			Self::decimals {} => EvmDataWriter::new_with_selector(826074471u32).build(),
			Self::eip2612_domain_separator {} => {
				EvmDataWriter::new_with_selector(910484757u32).build()
			}
			Self::eip2612_nonces { owner } => EvmDataWriter::new_with_selector(2127478272u32)
				.write(owner)
				.build(),
			Self::eip2612_permit {
				owner,
				...
				.write(s)
				.build(),
			Self::freeze { account } => EvmDataWriter::new_with_selector(2367676207u32)
				.write(account)
				.build(),
			Self::freeze_asset {} => EvmDataWriter::new_with_selector(3566436177u32).build(),
			Self::mint { to, value } => EvmDataWriter::new_with_selector(1086394137u32)
				.write(to)
				.write(value)
				.build(),
			...
			Self::thaw { account } => EvmDataWriter::new_with_selector(1587675670u32)
				.write(account)
				.build(),
			Self::thaw_asset {} => EvmDataWriter::new_with_selector(1374431959u32).build(),
			Self::total_supply {} => EvmDataWriter::new_with_selector(404098525u32).build(),
			Self::transfer { to, value } => EvmDataWriter::new_with_selector(2835717307u32)
				.write(to)
				.write(value)
				.build(),
			Self::transfer_from { from, to, value } => {
			...
			Self::transfer_ownership { owner } => EvmDataWriter::new_with_selector(4076725131u32)
				.write(owner)
				.build(),
			Self::__phantom(_, _) => ::core::panicking::panic_fmt(::core::fmt::Arguments::new_v1(
				&["__phantom variant should not be used"],
	...
	Runtime: Get<u32>,
{
	...
	match (
		&"(address,address)",
		&<(Address, Address) as EvmData>::solidity_type(),
	) {
		(left_val, right_val) => {
	...
	match (
		&"(address,uint256)",
		&<(Address, U256) as EvmData>::solidity_type(),
	) {
		(left_val, right_val) => {
			if !(*left_val == *right_val) {
				let kind = ::core::panicking::AssertKind::Eq;

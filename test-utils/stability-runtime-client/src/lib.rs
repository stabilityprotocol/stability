...
impl sc_executor::NativeExecutionDispatch for LocalExecutorDispatch {
	...
	fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
		stability_test_runtime::api::dispatch(method, data)
	}

	fn native_version() -> sc_executor::NativeVersion {
		stability_test_runtime::native_version()
	}
}

/// Test client database backend.
pub type Backend = stability_test_client::Backend<stability_test_runtime::Block>;
...
pub type TestClientBuilder<E, B> = stability_test_client::TestClientBuilder<
	stability_test_runtime::Block,
	E,
	B,
	GenesisParameters,
...
impl<B> TestClientBuilderExt<B>
	for TestClientBuilder<
		...
		B,
	> where
	B: sc_client_api::backend::Backend<stability_test_runtime::Block> + 'static,
{
	fn genesis_init_mut(&mut self) -> &mut GenesisParameters {

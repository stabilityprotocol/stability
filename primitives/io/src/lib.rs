...
pub trait Crypto {
	...
	/// Returns all `ecdsa` public keys for the given key id from the keystore.
	fn eth_ecdsa_public_keys(&mut self, id: KeyTypeId) -> Vec<ecdsa::Public> {
		Self::ecdsa_public_keys_version_1(self, id)
	}
	...
	///
	/// Returns the public key.
	fn eth_ecdsa_generate(&mut self, id: KeyTypeId, seed: Option<Vec<u8>>) -> ecdsa::Public {
		Self::ecdsa_generate_version_1(self, id, seed)
	}
	...
	fn eth_ecdsa_sign(
		...
		Self::ecdsa_sign_version_1(self, id, pub_key, msg)
	}
	...
	///
	/// Returns the signature.
	fn eth_ecdsa_sign_prehashed(
		&mut self,
		id: KeyTypeId,
		pub_key: &ecdsa::Public,
		msg: &[u8; 32],
	) -> Option<ecdsa::Signature> {
		Self::ecdsa_sign_prehashed_version_1(self, id, pub_key, msg)
	}
	...
	/// Returns `true` when the verification was successful.
	/// This version is able to handle, non-standard, overflowing signatures.
	fn eth_ecdsa_verify(sig: &ecdsa::Signature, msg: &[u8], pub_key: &ecdsa::Public) -> bool {
		Self::ecdsa_verify_version_1(sig, msg, pub_key)
	}
	...
	#[version(2)]
	fn eth_ecdsa_verify(sig: &ecdsa::Signature, msg: &[u8], pub_key: &ecdsa::Public) -> bool {
		Self::ecdsa_verify_version_2(sig, msg, pub_key)
	}

	/// Verify `ecdsa` signature with pre-hashed `msg`.
	///
	/// Returns `true` when the verification was successful.
	fn eth_ecdsa_verify_prehashed(
		sig: &ecdsa::Signature,
		msg: &[u8; 32],
		pub_key: &ecdsa::Public,
	) -> bool {
		Self::ecdsa_verify_prehashed_version_1(sig, msg, pub_key)
	}
	...
	///
	/// Returns `true` when the verification is either successful or batched.
	fn eth_ecdsa_batch_verify(
		&mut self,
		sig: &ecdsa::Signature,
		msg: &[u8],
		pub_key: &ecdsa::Public,
	) -> bool {
		self.ecdsa_batch_verify_version_1(sig, msg, pub_key)
	}

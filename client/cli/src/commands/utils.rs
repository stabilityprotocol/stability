		...
		match output {
			OutputType::Json => {
				let json = json!({
					...
					"ss58Address": <Pair::Public as ByteArray>::to_raw_vec(&pair.public()),
				});
				println!(
					"{}",
			...
			OutputType::Text => {
				println!(
					...
					<Pair::Public as ByteArray>::to_raw_vec(&pair.public()),
				);
			}
		}
...
macro_rules! with_crypto_scheme {
	...
	(
		...
		match $scheme {
			...
			$crate::CryptoScheme::Ethereum => {
				$method::<account::EthereumSignaturePair, $($generics),*>($($params),*)
			}
		}
	};
}

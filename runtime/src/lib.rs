...
impl_runtime_apis! {
	...
	impl rpc_eth_extension_api::EthExtensionRpcApi<Block> for Runtime {
		fn convert_delegated_transaction(transaction: EthereumTransaction, meta_trx_nonce : u64, meta_trx_sponsor: H160, meta_trx_sponsor_signature: Vec<u8>) -> <Block as BlockT>::Extrinsic {
			UncheckedExtrinsic::new_unsigned(
				pallet_meta_transactions::Call::<Runtime>::send_delegated_transaction { transaction, meta_trx_nonce, meta_trx_sponsor, meta_trx_sponsor_signature }.into(),
			)
		}
